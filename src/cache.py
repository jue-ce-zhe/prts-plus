...
def load_mapping(mapping_file: str) -> Dict[str, str]:
    ...
    """
    try:
        with open(os.path.join(RESOURCE_PATH, mapping_file), 'r', encoding='utf-8') as file:
            return json.load(file)
    except FileNotFoundError:
        logger.error(f'{mapping_file} not found')
        raise FileNotFoundError(f'{mapping_file} not found')

def load_resource(resource_name: str, resource_mapping: Dict[str, str], resource_path: str, load_func: Callable[[List[str]], Any]) -> Any:
    """
    ...
    if resource_name not in resource_mapping:
        logger.error(f'No resource found for name: {resource_name}, please check if the name is valid')
        raise ValueError(f'No resource found for name: {resource_name}, please check if the name is valid')
...
LEVEL_MAPPING: Dict[str, str] = load_mapping('level_mapping.json')
...
def load_avatars(oper_name: str) -> None:
    ...
    avatars[oper_name] = load_resource(oper_name, OPERATOR_MAPPING, os.path.join(RESOURCE_PATH, 'avatar'), load_func)
    logger.info(f'Loaded avatars for {oper_name}')

def load_map(map_code: str) -> None:
    def load_func(paths: List[str]) -> Dict[str, Any]:
        with open(paths[0], 'r', encoding='utf-8') as file:
            return json.load(file)
    maps[map_code] = load_resource(map_code, LEVEL_MAPPING, os.path.join(RESOURCE_PATH, 'map'), load_func)
    logger.info(f'Loaded avatars for {map_code}')

def get_avatars(oper_name: str) -> List[np.ndarray]:
    return get_resource(oper_name, avatars, load_avatars)

def get_map(map_code: str) -> Dict[str, Any]:
    return get_resource(map_code, maps, load_map)
